...
export const useConnectionStore = create<ConnectionState>((set: any) => ({
  ...
  setConnectionParams: (ip: string, port: number, token: string) => set({ hostIp: ip, port, authToken: token }),
  ...
  updateEdgeState: (key: string, value: any) => set((state: ConnectionState) => ({
    edgeState: {
      ...state.edgeState,
      [key]: value
    }
